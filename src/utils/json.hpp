...
template <class T> optional<T> json_get(const nlohmann::json& j, string_view key) {
...
template <class T>T json_get_or(const nlohmann::json& j, string_view key, T defv) {
    if (auto v = json_get<T>(j, key)) return *v;
    return defv;
}