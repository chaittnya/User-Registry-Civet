...
// Generate random 8-char hex suffix for names
std::string random_hex8(std::mt19937 &rng) {
    static const char hex[] = "0123456789abcdef";
    std::uniform_int_distribution<int> dist(0, 15);
    std::string s(8, '0');
    for (auto &ch : s) ch = hex[dist(rng)];
    return s;
}
...
int main() {
    ...
    if (!dsn) {
        std::cerr << "Please set DATABASE_URL environment variable.\n";
        return 1;
    }
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        std::cerr << "Prepare failed: " << PQerrorMessage(conn) << "\n";
        PQclear(res);
        PQfinish(conn);
        return 1;
    }
    ...
    for (auto &u : users) {
        ...
        if (PQresultStatus(res) == PGRES_COMMAND_OK ||
            ...
            std::cerr << "Insert failed for " << u.first << ": "
                      << PQerrorMessage(conn);
        }
        PQclear(res);
    }
    ...
    std::cout << "Inserted " << inserted << " users successfully.\n";
    PQfinish(conn);
    return 0;
}
